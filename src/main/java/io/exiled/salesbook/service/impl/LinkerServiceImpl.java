...
public class LinkerServiceImpl implements LinkerService {
    @Override
    public List<Linker> buildLinks(String link) {
        List<Linker> ll = new ArrayList<>();
        ll.add(new Linker("aa","bb"));
        ll.add(new Linker("cc","dd"));
        return ll;
    }
}
//    //    @Value("${photory.path}")
...
////        }
//        try {
////            File fileName = new File(updatedLink);
////            File[] fileList = fileName.listFiles();
////
////            for (File file : fileList) {
//////                new Linker(file.toString().substring(fullPath.length() + 1), file.toString().substring(fullPath.length() + 1));
////                new Linker("link A", "link B");
////            }
//        } catch (Exception e) {
